...
def process_immigration_data(spark, input_data, output_data, file_name, temperature_file, mapping_file):
    ...
    dim_df = helpers.create_country_dimension_table(spark, immigration_df, temp_df, output_data, mapping_file)

    # create immigration fact table
    fact_df = helpers.create_immigration_fact_table(spark, immigration_df, output_data)
...
def process_demographics_data(spark, input_data, output_data, file_name):
    ...
    new_demographics_df = util.clean_spark_demographics_data(demographics_df)

    # create demographic dimension table
    df = helpers.create_demographics_dimension_table(new_demographics_df, output_data)
...
def main():
    ...
    input_data = "s3://capstonesparkprojectdata/"
    output_data = "s3://capstonesparkprojectdata/"
